...
export const testStateTE = (f: (exp: Matcher) => void) => <T>(req: TE.TaskEither<ExecuteError, M.ExecuteResult<T>>) =>
  pipe(
    req,
    TE.map((a) => {
      f(expect(a.state));
...
export const testCacheTE =
  (f: (exp: DriveLookup.Cache) => void) => (req: TE.TaskEither<ExecuteError, M.ExecuteResult<any>>) =>
    pipe(
      req,
      TE.map((a) => {
        f(a.state.cache);
...
export const testCallsTE = (toMatch: CallsPartial) => <T>(req: TE.TaskEither<ExecuteError, M.ExecuteResult<T>>) =>
  pipe(
    req,
    TE.map((a) => {
...
export const testCallsErrorTE = (toMatch: CallsPartial) => <T>(req: TE.TaskEither<ExecuteError, M.ExecuteResult<T>>) =>
  pipe(
    req,
    TE.mapLeft((a) => {
      expect(a.calls()).toMatchObject(toMatch);
...
export const testErrorIs =
  (isError: (e: Error) => boolean) => (req: TE.TaskEither<ExecuteError, M.ExecuteResult<any>>) =>
    pipe(
      req,
      testError,
      TE.mapLeft((a) => {
...
export const testResTE = <T>(f: (res: T) => void) => (req: TE.TaskEither<ExecuteError, M.ExecuteResult<T>>) =>
  pipe(
    req,
    TE.map((a) => {
      f(a.res);
...
type Matcher = ReturnType<typeof expect>;

export const testExpectResTE =
  (f: (exp: Matcher) => void) => (req: TE.TaskEither<ExecuteError, M.ExecuteResult<any>>) =>
    pipe(
      req,
      TE.map((a) => {
        f(expect(a.res));
...
const testCalls =
  (toMatch: Partial<ReturnType<M.Calls["calls"]>>): CallsMatcherF => (calls: ReturnType<M.Calls["calls"]>) => {
    expect(calls).toMatchObject(toMatch);
  };

const testRes = (f: (exp: Matcher) => void): ResultMatcherF => (res) => f(expect(res));
