...
export const logTimeIO = (logger: Logger) => (name: string) => <A>(f: IO.IO<A>): IO.IO<A> =>
  pipe(
    timeIO(f),
...
export const logTimeTE = (logger: Logger) => (name: string) => <E, A>(te: TE.TaskEither<E, A>): TE.TaskEither<E, A> => {
  return pipe(
    timeTE(te),
